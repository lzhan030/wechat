...
<div class="box">
	...
	<a href="<?php echo $urllink;?>" class="box-item">
		...
		<i style="background:url(<?php if((empty($nav['icon']))||(stristr($nav['icon'],"http")!==false)){echo $nav['icon'];}else{echo $upload['baseurl'].$nav['icon'];}?>) no-repeat;background-size:cover;" class="img-rounded"></i>
		<?php } else { ?>
		<i class="glyphicon <?php echo $nav['css']['icon']['icon'];?> img-rounded" style="<?php echo $nav['css']['icon']['style'];?>"></i>
		<?php } ?>
