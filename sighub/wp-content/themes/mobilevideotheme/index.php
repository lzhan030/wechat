...
<html>
	...
	<script type="text/javascript">
		$(function(){
			...
			var ajax_option={			
				url:"<?php bloginfo('template_directory'); ?>/mobile_post.php?action="+actionparm+"&gweid=<?php echo $gweidt;?>"+"&siteId=<?php echo $siteId;?>"+"&redirect_url=<?php echo urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'])?>#wechat_redirect",
		...
		$count = 0; ?>
			...
			?>
                <div <?php post_class(); ?> class="post" id="post-<?php the_ID(); ?>">
                    ...
                    <?php } ?>
                    <div class="title">
                       <h2><a href="<?php the_permalink(); echo '&site='.$siteId.'&gweid='.$gweidt; ?>#wechat_redirect" title="<?php the_title(); ?>"><?php the_title(); if( is_sticky() ) echo '&nbsp;&nbsp;<span style="color:red;">置顶</span>'; ?></a></h2>
                    </div>
...
<br/><br/>
      ...
      <?php if($isPostPermission == 'true'){ ?>
		<div id="respond" style="margin-left: 5px;">
			<!--<a href="<?php bloginfo('template_directory'); ?>/video_upload_fast.php?gweid=<?php echo $gweidt;?>&siteId=<?php echo $siteId;?>&redirect_url=<?php echo urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'])?>#wechat_redirect">上传视频</a>-->
			<input class="publicuploadvideo" name="submit" type="button" id="publicuploadvideo" tabindex="5" onclick="location.href='<?php bloginfo('template_directory'); ?>/video_upload_fast.php?gweid=<?php echo $gweidt;?>&siteId=<?php echo $siteId;?>&redirect_url=<?php echo urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'])?>#wechat_redirect'" value="上传视频" />
		</div>
       <?php }?>
