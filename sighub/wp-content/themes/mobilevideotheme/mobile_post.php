...
if( $_GET['action']=="mobiletheme" ){
	...
	$needapproval = getSiteMeta('mobilethemeNeedApproval', $siteId);
	...
	*/
	...
	if(($selCheck['wechatvip']==1)&&(($isShowVipmember_editor=='true')&&(empty($memberinfo)))){
		...
		$blogContent=$_POST['video_pkey'];
		$blogAuthor=$_POST['blogAuthor'];
		$postSiteId = $_GET['siteId'];
		...
		$result=$wpdb->query( $wpdb->prepare("INSERT INTO {$wpdb->prefix}posts(post_author,post_date,post_date_gmt,post_content,post_title,post_excerpt,post_name,to_ping, pinged, post_content_filtered,post_status)VALUES (%s,now(),utc_timestamp(),%s,%s,%s,%s,%s,%s,%s,%s)",$userId,$blogContent,$blogTitle,'',$blogAuthor,'','',$postSiteId,$post_status));
