...
<html>
	...
	<body>
		<div class="main_auto">
				...
				<div  style="width:60%">
					<form action="" method="post">
						<td>						
                           ...
                           onclick="">
							<input type="submit" name="clear_news" onclick="return confirm('确定删除?');" class="btn btn-default" value="删除"></a>						
			...
			<div class="pic">
				...
				<input type="submit" onclick="return confirm('确定删除?');" name="clear_text" class="btn btn-default" value="删除" style="margin-top:10px;margin-bottom:20px"/>
				<?php 
				if($mtext!=null){
					foreach($mtext as $text){
						$texts=stripslashes($text->text_content);
					} 
					echo "<textarea id='editor_id' name='sendContent' cols='42' rows='11' style='width: 650px;height:215px' >".$texts."</textarea>";
				}else{
					echo "<textarea id='editor_id' name='sendContent' cols='42' rows='11' style='width: 650px;height:215px' ></textarea>";
				}
				?>					
				</form>	
                <span>注意：文本要小于2048字节（对应682个中文汉字)</span>				
			</div>
		</div>
	...
	<?php
		...
		function selectNews(newsid){	
			window.open('autoreply_image_select.php?beIframe&artType=post&selectnewsid='+newsid,'_blank','height=600,width=800,top=80,left=240,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no,titlebar=no')
		}
		...
		function useText(){
			...
			if(content!=""){
				str = content.replace(/<[^>].*?>/g,"");
			     jQuery.post(
			        "<?php echo constant("CONF_THEME_DIR"); ?>/wechat/autorep/autoreply_text_insert.php?beIframe",
			        {content : content },
