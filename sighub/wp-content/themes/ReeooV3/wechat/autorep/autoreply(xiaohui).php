...
include '../common/wechat_dbaccessor.php';
 ?>
<?php	
	//$mes_list=wechat_mess_kw_get("subscribe",$current_user->ID);
    //20140623 janeen update
    //$mes_list=wechat_mess_kw_get("subscribe",$_SESSION['WEID'] );
	  $mes_list=wechat_mess_kw_get_group("subscribe",$_SESSION['GWEID'] );
	//end
	...
	if(isset($_POST['clear_news']))
    {
       ...
       //20140623 janeen update
       //$b = wechat_clearNews("subscribe",$_SESSION['WEID']);
	    $b = wechat_clearNews_group("subscribe",$_SESSION['GWEID']);
	   //end
        echo '<script>';
        if($b)
            echo "alert('删除成功!');";
	...
	if($is_select==1){
		...
		//20140623 janeen update
		//$mes=wechat_mess_kw_get("subscribe",$_SESSION['WEID'] );
...
<html>
	...
	<body>
		<div class="main_auto">
			<div class="main-title">
				<div class="title-1">当前位置：首次关注应答> <font class="fontpurple">应答内容设置 </font>
				</div>
			</div>
			<div class="bgimg"></div>
				...
				<div  style="width:60%">
					<form action="" method="post">
						<td>						
							...
							<a href="<?php echo get_template_directory_uri(); ?>/wechat/autorep/autoreply.php?beIframe&tab=1"
                           onclick="">
							<input type="submit" name="clear_news" class="btn btn-default" value="删除"></a>						
			...
			<div class="pic">
				<form action="autoreply_text_insert.php?beIframe" method="post">				 
	...
	<?php
		...
		if($newsid!=null){
			...
			//20140623 janeen update
			//$reply=wechat_get_aplymesgid("subscribe",$_SESSION['WEID']);
			$reply=wechat_get_aplymesgid_group("subscribe",$_SESSION['GWEID']);
			//end
			foreach($reply as $ry){
				$rey=$ry->arplymesg_id;
			}
			...
			for(var i=0; i<aCheckBox.length; i++){
				if(aCheckBox[i].getAttribute('type')=='checkbox'){
					if(aCheckBox[i].checked==true){
						...
						location.href="/wp-content/themes/ReeooV3/wechat/autorep/autoreply_image_insert.php?beIframe&netId="+nid;
		...
		function selectNews(newsid){	
			//onclick="javascript:window.showModalDialog('<?php echo get_template_directory_uri(); ?>/wechat/autorep/autoreply_image_select.php?beIframe&artType=post','','dialogHeight:520px;dialogWidth:800px;status:0;edge:sunken');
			//window.open('autoreply_image_select.php?beIframe&artType=post','_blank','height=520,width=800,top=120,left=240,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no,titlebar=no')
			
			window.open('autoreply_image_select.php?beIframe&artType=post&selectnewsid='+newsid,'_blank','height=520,width=800,top=120,left=240,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no,titlebar=no')
		}
		...
		function useNews(){
			...
			if(newid!=-1){
				...
				xmlHttp.open("GET","<?php echo constant("CONF_THEME_DIR"); ?>/wechat/autorep/autoreply_image_insert.php?beIframe&netId="+newid,true);
				xmlHttp.onreadystatechange = function(){
					if(xmlHttp.readyState == 4 && xmlHttp.status == 200){
						alert("保存成功!");
						var url="<?php echo get_template_directory_uri(); ?>";
						location.href=url+'/wechat/autorep/autoreply.php?beIframe&tab=1';
					}
				}
				xmlHttp.send(null);	
			}else{
		...
		function useText(){
			...
			if(content!=""){
				createXMLHttpRequest();
				xmlHttp.open("GET","<?php echo constant("CONF_THEME_DIR"); ?>/wechat/autorep/autoreply_text_insert.php?beIframe&content="+escape(content),true);
				xmlHttp.onreadystatechange = function(){
					if(xmlHttp.readyState == 4 && xmlHttp.status == 200){
						alert("保存成功!");
						var url="<?php echo get_template_directory_uri(); ?>";
						location.href=url+'/wechat/autorep/autoreply.php?beIframe&tab=0';
					}
				}
				xmlHttp.send(null);	
			}else{
