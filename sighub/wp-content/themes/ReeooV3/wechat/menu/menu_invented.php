...
<html>
	...
	<title>菜单管理</title>
	<style>
		.summary{float:left;width:96%;margin-top:10px;}
		.summary_ul1{background-color:#60d295;border:1px solid #57c78b;color:white;}
		.summary_font{font-family:"微软雅黑";}
	</style>
	</head>
	<body>
	<div class="main_auto">
		<div class="main-title" style="width:100%;height:auto;">
			<div class="title-1">当前位置：共享微信菜单管理> <font class="fontpurple">菜单内容设置 </font></div>
			<div class="title-1" style="height:auto;width:100%; clear:both;">
				<div class="summary">
					<ul class="summary_ul1" style="height:auto">
						<li>
							<?php
									$wechat_group=getWechatGroupInfo_gweid($_SESSION['GWEID']);//这个为激活的号
							...
							?>
							<font class="summary_font" style="font-size:15px;font-weight:normal;margin-left:20px;">该微信菜单在以下几个微信公众号下使用:&nbsp;
								<?php					
								$gweids=getWechatGroupInfo_gweid_activeshared($user_id);//所有共享号
								$first = true;
								foreach($gweids as $gweidinfo){
									$gweidzero=$gweidinfo->GWEID;//共享号的gweid
									$usechatinfo=web_admin_usechat_allpriinfo_group($gweidzero);//私有服务号或者私有认证订阅号并为共享号所使用的微信号
			...
			<div id="primary" class="site-content" style="clear:both;">
				<div id="content" role="main">
					<input type="button" onclick="deletemenu()" class="btn btn-default" value="清空菜单" style="width:120px;float:right;margin-top:5px;margin-right: 8px;" />	
					<input type="button" onclick="uploadmenu()" class="btn btn-primary" value="上传菜单" style="width:120px;float:right;margin-top:5px;margin-right: 20px;" />
				</div>
			</div>	
		</div>
		<div>
			<div class="main-new" style="margin-bottom:10px; margin-left:0; width:100%;">
			</div>
			<div  style="float:left;">
				<input class="menu btn btn-primary" type="button" value="添加根菜单" style="margin-bottom:10px; position:relative;width: 120px;" />
				<ul id="ttre" class="easyui-tree"></ul>
			</div>
			<div class="main keyWordMain" style="margin-left:150px; width:700px;height:700px;margin-top: 50px; padding: 10px 20px;">
				<div id="nav-main" class="navs" style="margin-bottom:10px; width:650px;">
					<ul class="nav nav-tabs" style="padding-left:0;height:42px">
				...
				<div class="webUrl">				
					<div class="nub-1">
						<input class="urlconnect btn btn-primary" type="button" value="保存" style="margin-bottom:30px" />
						<input class="urlunconnect btn btn-default" type="button" value="删除" style="margin-bottom:30px"/>	
						<p style="margin-bottom:10px;"><b>添加链接:</b></p>
						...
						<input id="siurl" type='text' name='meniUrl' value='' class="form-control" style="margin-top:5px;" readonly='true'/>
						<!--添加外联-->
						<br/>
	...
	function selectNews(){	   
			if(menuId==undefined){
				...
				window.open('menu_news_select_invented.php?beIframe&menuId='+menuId+'&menuType='+menuType+'&menuKey='+menuKey+'&menuPad='+menuPad,'_blank','height=520,width=800,top=120,left=240,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no,titlebar=no')
			}
		}
		function uploadmenu(){
			$.ajax({
				url: "<?php echo constant("CONF_THEME_DIR");  ?>/wechat/menu/menu_create_invented.php?beIframe",  
				type: "POST",
				dataType: 'json',
				error: function(){  
		...
		function deletemenu(){
			if(confirm("确定删除吗？")){	
				$.ajax({
					url: "<?php echo constant("CONF_THEME_DIR");  ?>/wechat/menu/menu_delete_invented.php?beIframe",  
					type: "POST",
					dataType: 'json',
					error: function(){  
						alert('Error loading XML document');  
					...
					success: function(data,status){//如果调用php成功
						...
						location.href=url+'/wechat/menu/menu_invented.php?beIframe';
					}
				});
			}
		}		
	...
	//循环出所有多图文个数的框架		
				...
				if(menuPad!=-1)	{
					...
					jQuery.post(
				        ...
				        function(data, textStatus, jqXHR){
							alert("保存成功");
							var url="<?php echo get_template_directory_uri(); ?>";
							location.href=url+'/wechat/menu/menu_invented.php?beIframe&menusecid='+menuId;	
				        },
				        "text"
				    );	
				}else{					
		...
		$(".urlunconnect").click(function(e){			
			...
			if(menuType=="view")	{
				if(confirm("确定删除吗？")){
					createXMLHttpRequest();
					xmlHttp.open("GET","<?php echo constant("CONF_THEME_DIR"); ?>/wechat/menu/menu_content_delete.php?beIframe&menuId="+menuId+"&menuType="+menuType+"&menuName="+menuName+"&menuKey="+menuurl+"&menuPad="+menuPad,true);
					xmlHttp.onreadystatechange = function(){
						if(xmlHttp.readyState == 4 && xmlHttp.status == 200){
							alert("删除成功");
							var url="<?php echo get_template_directory_uri(); ?>";
						...
						if (item.id=='新建'){ 
							if(fid==n1){
								if(c1>4){
									...
									window.open("add_menu_dlg.php?menuId="+tcurrentId+"&parid="+tparenId,"_blank","height=320,width=800,top=120,left=240,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no,titlebar=no");
								}
							}else if(fid==n2){
								if(c2>4){
									alert("最多建五个子菜单");
							...
							window.open("update_menu_dlg.php?menuId="+tcurrentId,"_blank","height=320,width=800,top=120,left=240,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no,titlebar=no");
						}
						else{
							if(confirm("确定删除吗？")){
								$.ajax({
