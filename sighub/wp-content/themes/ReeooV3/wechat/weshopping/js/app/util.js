		...
		if(!elm.editor) {
			require(['editor'], function(){
				var editor = tinyMCE.createEditor(id, {
					...
					toolbar2: "code print | styleselect link image multiimage media emoticons",
					language: 'zh_CN',
		...
		var content =
			...
			'<div class = "tab-content form-horizontal" style="padding:20px 0;">'+
				...
				'<div class="tab-pane" id="image_remote">'+
					...
					'<form action="./index.php?c=utility&a=file&do=upload&type=image&callback=uploaderRemoteImageComplete" enctype="multipart/form-data" method="post" target="__image_file_remote_uploader">'+
						'<div class="form-group">' +
							'<label class="col-xs-12 col-sm-2 control-label">上传图片</label>' +
							'<div class="col-sm-10">' +
								'<input type="file" name="file">'+
		...
		modalobj.find('button.btn-browser').on('click', function(){
			...
			window.imageBrowser = {
				...
				browser: function(path){
					...
					dialog.find('.modal-body').load('./index.php?c=utility&a=file&do=browser&options='+base64options+'&type=image&callback=imageBrowser&path=' + path);
				},
				select: function(r) {
				...
				delete: function(file){
					$.get('./index.php?c=utility&a=file&do=delete&type=image&options='+base64options+'&file=' + file).success(function(dat){
						if(dat == 'success') {
