...
<link type="text/css" rel="stylesheet" href="<?php bloginfo('template_directory'); ?>/wechat/weshopping/css/goods_option.css">
<table class="table">
	<tr>
...
<script language="javascript">
	...
	for(var i=0;i<newlen;i++){
		...
		var val ={ id : "",title:titles, stock : "",costprice : "",productprice : "",marketprice : "",point : "",weight:"" };
		if( $(".option_id_" + ids).length>0){
			val ={
				...
				point : $(".option_point_" + ids+":eq(0)").val(),
		...
		hh += '<td class="info"><input name="option_point_' +ids+'[]" type="text" class="form-control option_point option_point_' + ids +'" " value="' +(val.point=='undefined'?'':val.point )+'"/></td>';
		hh += '<td class="success"><input name="option_weight_' + ids +'[]" type="text" class="form-control option_weight option_weight_' + ids +'" " value="' +(val.weight=='undefined'?'':val.weight )+'"/></td>';
		hh += "</tr>";
	}
	html+=hh;
	html+="</table>";
...
function picktrue(obj,specitemid) {
		...
		editor.loadPlugin('image', function() {
			editor.plugin.imageDialog({
					...
					clickFn : function(url, title, width, height, border, align) {
						KindEditor('#specitem'+specitemid).val(url);
						KindEditor("#specitemimg"+specitemid).attr("src",url);
						KindEditor("#itempicurl"+specitemid).show();
						editor.hideDialog();
...
/*删除图片*/ 
function delItemImage(specitemid){
	$("#itempicurl"+specitemid).css('display','none');
	$('#specitem'+specitemid).val('');
	$("#specitemimg"+specitemid).attr("src",'');	
}
</script>