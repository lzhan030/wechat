...
include 'nokeyword_permission_check.php';
 ?>
<?php	
...
<html>
					...
					<form action="" method="post">
						<td>
                            ...
                            <a href="<?php echo get_template_directory_uri(); ?>/wechat/nokeyword/nokeyword.php?beIframe&tab=1"
                                   onclick="">
								<input type="submit" onclick="return confirm('确定删除?');" name="clear_news" class="btn btn-default" value="删除">
							</a>
	...
	<?php
		...
		function selectNews(newsid){	
			window.open('nokeyword_image_select.php?beIframe&artType=post&selectnewsid='+newsid,'_blank','height=600,width=800,top=80,left=240,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no,titlebar=no')
		}
	...
	function useNews(){
		...
		function useText(){
			...
			if(content!=""){
				...
				createXMLHttpRequest();
			jQuery.post(
		        "<?php echo constant("CONF_THEME_DIR"); ?>/wechat/nokeyword/nokeyword_text_insert.php?beIframe",
		        {content : content },
