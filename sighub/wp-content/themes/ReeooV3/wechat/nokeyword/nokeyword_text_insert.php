...
get_header(); 
 ...
 <?php	
    ...
    {	
		$b = wechat_clearNews_group('nokey',$_SESSION['GWEID'],'weChat_text');
		 echo '<script>';
        if($b)
            echo "alert('取消成功!');";
        else
	...
	//将文本内容插入素材文本表，并拿到文本id		
	
	
	//找到当前用户对应的subs记录，如果没有就添加，并返回id
	$mes=wechat_mess_kw_get_group('nokey',$_SESSION['GWEID']);
	if($mes!=null){
		foreach($mes as $messa){
			//判断有关注回复记录
			$arp_id=$messa->arply_id;
			$text=wechat_autrplay_text_get_group("nokey",$_SESSION['GWEID']);
			if($text!=null){
				foreach($text as $t){
					$text_id=$t->text_id;
				}
				...
				if($aty_upt===false){
					...
					echo "成功设置该文本为无匹配回复内容";
				}
				
			}else{
				//如果没有文本类型的关注回复，就添加
				$text_id=wechat_autrplay_text_insert_group("nokey",$content,$currentuser,$_SESSION['GWEID']);
				$aty_upt=wechat_autrplay_acty($text_id,"weChat_text",$arp_id);
				if($aty_upt===false){
					echo "内容更新失败！";
				}else{
		...
		//如果没有文本类型的关注回复，就添加
		$text_id=wechat_autrplay_text_insert_group("nokey",$content,$currentuser,$_SESSION['GWEID']);
		$insert=wechat_mess_kw_add_group("weChat_text",$text_id,"nokey",$currentuser,$_SESSION['GWEID']);
		if($insert===false){
			echo "内容添加失败！";
		}else{
			echo "成功设置该文本为无匹配回复内容";
