...
class WxwallModuleProcessor extends WeModuleProcessor {
	...
	private function register() {
		...
		$this->beginContext();
		pdo_update('wxwall_members', array('rid' => $rid, 'isblacklist' => 0, 'lastupdate' => TIMESTAMP), array('from_user' => $this->message['from']));
	...
	protected function beginContext($expire = 3600) {
		...
		$member = pdo_fetch("SELECT id FROM ".tablename('wxwall_members')." WHERE from_user = '{$this->message['from']}' LIMIT 1");
		if (!empty($member)) {
	...
	private function getMember() {
		$member = (array)pdo_fetch("SELECT id, lastupdate, rid FROM ".tablename('wxwall_members')." WHERE from_user = '{$this->message['from']}' LIMIT 1");
