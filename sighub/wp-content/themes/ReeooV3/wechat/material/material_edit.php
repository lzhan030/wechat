...
//如果当前号是虚拟号，则不显示微官网站点选择和微学校
$isvirtualgweid = web_admin_function_virtual_gweid($GWEID);
?>
...
<html>
	...
	<script src="<?php bloginfo('template_directory'); ?>/js/jquery-1.11.1.min.js?t=123" type="text/javascript"></script>
...
<script src="<?php bloginfo('template_directory') ?>/js/tinymce/tinymce.min.js"></script>
	...
	<style>
		...
		#itemDes {overflow:auto; height:260px; border: 1px solid #ccc;}
	...
	<body>
			...
			<div class="main_bd" >			
				<div class="left" style=" margin-left: 0%; ">
					<div  id="pre-title0">
						<div  myurl="" class="pre-bg" >
						 ...
						 <textarea class="newsAbs" type="text" style="display:none" value=""></textarea>
				...
				<div class="right" style="background: rgb(250, 250, 250);">
					<form>
						...
						<label for="content">摘要（选填,该摘要只在发送图文消息为单条时显示（建议不超过300个汉字））</label>
						<div>
							<textarea id="itemAbs" name="abs"  class="form-control" style="width:390px;height:80px;" oninput="textPreview()"></textarea>
						</div>						
						<label for="content">正文</label>
						<div>
							<textarea id="itemDes" style="height:150px;" name="rule"></textarea>
						</div>
						<!--添加外联-->
						...
						<!--添加内联-->
						<div style="<?php if( $isvirtualgweid == 1 ) echo "display:none"; ?>">
						...
						<div style="<?php if( !$wechatactivity_vip && $isvirtualgweid == 0) echo "display:none"; ?>">
						<input type="radio" name="itmesUrl" value="2" onclick="textPreview()">
						<span> 会员中心</span>
						<input style="visibility:hidden;height:0px" type="text" name="memurl" id="memurl" value="<?php bloginfo('template_directory'); ?>/wesite/common/vip_detail.php" onchange="textPreview()" onpropertychange="textPreview()" oninput="textPreview()" onKeyDown="textPreview()" disabled="disabled"/>
						</div>
						
						<div style="<?php if( !$wechatschool || $isvirtualgweid == 1) echo "display:none"; ?>">
						<input type="radio" name="itmesUrl" value="3" onclick="textPreview()">
						<span> 微学校</span>
						<input style="visibility:hidden;height:0px" type="text" name="memwsurl" id="memwsurl" value="<?php echo home_url(); ?>/mobile.php?module=weSchool&do=index" onchange="textPreview()" onpropertychange="textPreview()" oninput="textPreview()" onKeyDown="textPreview()" disabled="disabled"/>
						</div>
		...
		<?php
			...
			/*如果没有http，证明为内链，加上home_url显示，插入时，判断有再截取入库*/
				...
				echo "itemAbs[".$o++."]=\"".str_replace ("\n" , '\n' , $material->news_item_abstract)."\";\n";
		...
		function tinymceOnInit(e){
			$(document).ready(function(e) {
			    ...
			    for(var k=0;k<n;k++){	
					...
					var newsAbs="#"+changeId+" textarea.newsAbs";
				...
				$(".left>div:not(#pre-add)").click(function(e) {
					...
					var no=$(this).children("textarea.newsAbs").val();
					...
					var vipmem='vip_detail';
					var schoollink='index';
					
					var videolink='videolist';
					var homeworklink='homeworklist';
					var noticelink='noticelist';
					...
					if(nu.indexOf(need)>=0){				
						...
						//$("#memurl").val("");
					...
					else if(nu.indexOf(videolink)>0){
							//$("#siurl").css("visibility","visible")
							$('input[type="radio"][value="3"]').prop("checked","checked");
							$("#siurl").val("");
							$("#sourl").val("");
							$("#memvurl").val(nu);
							document.getElementById("sourl").disabled=true;
							document.getElementById("siurl").disabled=true;
							document.getElementById("sitebutton").disabled=true;
					}else if(nu.indexOf(schoollink)>0){
							//$("#siurl").css("visibility","visible")
							$('input[type="radio"][value="3"]').prop("checked","checked");
							$("#siurl").val("");
							$("#sourl").val("");
					...
					else if(nu.indexOf(homeworklink)>0){
							$('input[type="radio"][value="4"]').prop("checked","checked");
							$("#siurl").val("");
							$("#sourl").val("");
							$("#memvurl").val("");
							$("#memhwurl").val(nu);
							...
							document.getElementById("siurl").disabled=true;
							document.getElementById("sitebutton").disabled=true;
					}else if(nu.indexOf(noticelink)>0){
							$('input[type="radio"][value="5"]').prop("checked","checked");
							$("#siurl").val("");
							$("#sourl").val("");
							$("#memvurl").val("");
							$("#memncurl").val(nu);
					...
					nd= nd.replace(reg,"\r\n");
					tinymce.get('itemDes').setContent(nd);
					//给右边图片显示内容
					if(pirl!=""){
						$("#pic").show(); 
						$("#pic").attr("src",pirl); 
				...
				$("#pre-add>a").click(function(e) {
					...
					//$("#memurl").val("");
					$("#itemAbs").val("");
					tinymce.get('itemDes').setContent("");
					$("#pic").attr("src","");
					$("#pic").hide();
			...
			$(".newsadd").click(function(e){		    
				...
				for(var i=0;i<$("div[id^=pre-title]").length;i++){
					if(("#"+$("div[id^=pre-title]")[i].id)!="#pre-titles"){
						...
						var itemsvabs=$("#"+$("div[id^=pre-title]")[i].id+" textarea.newsAbs").val();
						...
						itemAbs_temp=itemAbs_temp+itemsvabs+'|';
				...
				$.ajax({
						type:'post',
						url:"<?php echo constant("CONF_THEME_DIR"); ?>/wechat/material/material_upall.php?beIframe&itemId_save="+itemId_temp+"&itemTitle_save="+itemTitle_temp+"&itemAbs_save="+itemAbs_temp+"&itemDes_save="+itemDes_temp+"&itemUrl_save="+encodeURIComponent(itemUrl_temp)+"&newsId_save="+itemsvnid+"&material_name="+news_name+"&picUrl_save="+picUrl_temp,
						data:{'beIframe':'','itemId_save':itemId_temp,'itemTitle_save':itemTitle_temp,'itemAbs_save':itemAbs_temp,'itemDes_save':itemDes_temp,'itemUrl_save':itemUrl_temp,'newsId_save':itemsvnid,'material_name':news_name,'picUrl_save':picUrl_temp},
		...
		function textPreview(){
			...
			var newAbs=document.getElementById("itemAbs");
			...
			if(val==0){
				...
				//newUrl=document.getElementById("memvurl");
				newUrl=document.getElementById("memwsurl");
				document.getElementById("sourl").disabled=true;
				document.getElementById("siurl").disabled=true;
				document.getElementById("sitebutton").disabled=true;
			}
			...
			var nAbs=newAbs.value;
			var nDes=tinymce.get('itemDes').getContent();						
			nDes =  nDes.replace(/\n/g,'<br/>');			
			var nUrl=newUrl.value;
			for(var i=0;i<$("div[id^=pre-title]").length;i++){
				if($("#"+$("div[id^=pre-title]")[i].id+">.newsId").val()==$("#itemId").val()){	
					$("#"+$("div[id^=pre-title]")[i].id+" span.title1").text(ntext);
					$("#"+$("div[id^=pre-title]")[i].id+" textarea.newsAbs").val(nAbs);
					$("#"+$("div[id^=pre-title]")[i].id+" input.newsDes").val(nDes);
					console.info("#"+$("div[id^=pre-title]")[i].id+" input.newsDes");
					console.info("nDes:"+nDes);
