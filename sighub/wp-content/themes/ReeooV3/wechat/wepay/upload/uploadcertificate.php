...
class upfile{
    ...
    public $ph_name;   //上传文件名
    public $ph_tmp_name;    //文件临时文件名
    public $ph_path;    //上传文件存放路径
	public $ph_type;   //文件类型
   
    //public $al_ph_type=array('image/jpg','image/jpeg','image/png','image/pjpeg','image/gif','image/bmp','image/x-png');    //允许上传文件类型
   ...
   //获取文件类型
	...
	function up_file($file = null){
		...
		$uploadedfile['type'] = 'application/x-x509-ca-cert';
		...
		//var_dump("SELECT * FROM {$wpdb->prefix}wesite_space WHERE userid = {$id}");
	...
	function up_file1($file = null){
		...
		if ( ! function_exists( 'wp_handle_upload' ) ) 
			require_once( ABSPATH . 'wp-admin/includes/file.php' );
		$uploadedfile = ($file == null ? $_FILES['file1'] : $file);
		$uploadedfile['name'] = time().rand().strstr($uploadedfile['name'],'.');
		//$uploadedfile['type'] = 'image/jpeg';
		$upload_overrides = array( 'test_form' => false );
		$size = $uploadedfile['size']/1048576;	
