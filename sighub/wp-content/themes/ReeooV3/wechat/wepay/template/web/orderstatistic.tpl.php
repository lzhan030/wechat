...
<script type="text/javascript" src="<?php bloginfo('template_directory'); ?>/js/calendar.js"></script>
	...
	<!--主体-标题-->
		<div class="main-title">
			<div class="title-1">当前位置：<a href="<?php echo $this->createWebUrl('index',array());?>">微支付</a> > <a href="<?php echo $this->createWebUrl('indexstatistic',array('gweid' => $gweid));?>">统计图表</a> > <font class="fontpurple">订单统计 </font></div>
		</div>
		...
		<table  width="750" height="200" border="0" cellpadding="5px" style=" margin-left:30px; margin-top:30px;">
			...
			<tr>
				...
				<td width="250">
					<select class="form-control" style="margin-right:15px;" id="cate_1" name="cate_1" onchange="fetchChildCategory(this.options[this.selectedIndex].value);">
						...
						<!--微支付下商品类型-->
						<?php  if(is_array($wepaygoodsindexarray)) { foreach($wepaygoodsindexarray as $row) { 
									if($row['type']=='JSAPI'){
										$pay_type="[网页支付]";
						...
						?>
						<option value="<?php  echo "wepay".$row['id'];?>" ><?php  echo $pay_type.$row['goodsindex_name'];?></option>
						<?php  } } ?>
						...
						<option value="<?php  echo $row['id'];?>" ><?php  echo "[微商城]".$row['name'];?></option>
						<?php  } ?>
						<?php  } } ?>
					</select>
				</td>
			...
			<tr>
				<td colspan="3"><label for="name"><div class="help-block" style="margin-left:45px;">注:包括微支付模块和微商城模块通过微信支付生成的订单</div></label></td>
			</tr>
			<tr>
