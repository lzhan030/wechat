...
class WxPayHelper
{
	...
	*/
	...
	function deliver_goods($delivery_array){
		try {
			...
			$wxPayHelper->setParameter("openid", $delivery_array['openid']);
			$wxPayHelper->setParameter("transid", $delivery_array['transid']);
			$wxPayHelper->setParameter("out_trade_no", $delivery_array['out_trade_no']);
			$wxPayHelper->setParameter("deliver_timestamp", time());
			$wxPayHelper->setParameter("deliver_status", $delivery_array['deliver_status']);
			$wxPayHelper->setParameter("deliver_msg", $delivery_array['deliver_msg']);
			$wxPayHelper->setParameter("app_signature", $wxPayHelper->get_biz_sign($wxPayHelper -> parameters));
			$wxPayHelper->setParameter("sign_method", SIGNTYPE);
			
			
			$ch = curl_init();//初始化curl
			curl_setopt($ch, CURLOPT_URL,'https://api.weixin.qq.com/pay/delivernotify?access_token='.$jsondata->access_token);
			curl_setopt($ch, CURLOPT_HEADER, 0);//设置header
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);//要求结果为字符串且输出到屏幕上
			curl_setopt($ch, CURLOPT_POST, 1);//post提交方式
			curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($wxPayHelper -> parameters));
	...
	function order_query($order_query_array){
		...
		try {
			...
			$wxPayHelper->setParameter("appid", APPID);
			$wxPayHelper->setParameter("package","out_trade_no={$order_query_array['out_trade_no']}&partner=".PARTNERID."&sign=".strtoupper(md5("out_trade_no={$order_query_array['out_trade_no']}&partner=".PARTNERID."&key=".PARTNERKEY)));
			$wxPayHelper->setParameter("timestamp", time());
			$wxPayHelper->setParameter("app_signature", $wxPayHelper->get_biz_sign($wxPayHelper -> parameters));
			$wxPayHelper->setParameter("sign_method", SIGNTYPE);
			
			
			$ch = curl_init();//初始化curl
			curl_setopt($ch, CURLOPT_URL,'https://api.weixin.qq.com/pay/orderquery?access_token='.$jsondata->access_token);
			curl_setopt($ch, CURLOPT_HEADER, 0);//设置header
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);//要求结果为字符串且输出到屏幕上
			curl_setopt($ch, CURLOPT_POST, 1);//post提交方式
			curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($wxPayHelper -> parameters));
			...
			$data = json_decode($data['order_info']);
			return $data;
		}catch (SDKRuntimeException $e)
		{
			die($e->errorMessage());
	...
	function alarm($text){
		$alarm_obj = simplexml_load_string($text);
		return array(
			'ErrorType' => $alarm_obj -> ErrorType,
			'Description' => $alarm_obj -> Description,
			'AlarmContent' => $alarm_obj ->AlarmContent,
			'TimeStamp' => $alarm_obj ->TimeStamp,
		);
	}
