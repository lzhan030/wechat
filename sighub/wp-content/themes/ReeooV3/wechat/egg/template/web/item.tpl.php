...
<div class="alert alert-info reply-market-list" style="width:90%;">
	<table class="tb reply-news-edit" width="100%">
		<tr>
			<th>是否实物：</th>
			<td id="award-inkind">
				<span class="pull-right"><?php  if(empty($item)) { ?><a href="javascript:;" onclick="doDeleteItem('<?php  echo $itemid;?>')">删除</a><?php  } else { ?><a href="<?php echo $this->createWebUrl('delete', array('name' => 'egg', 'id' => $item['id']))?>" onclick="doDeleteItem('<?php  echo $itemid;?>')">删除</a><?php  } ?></span>
			...
			<td>
				<div class="item-line">
					<div class="item-input">
						<input type="text" class="form-control" value="<?php  echo $item['title'];?>" style="width:275px;" name="award-title<?php  echo $namesuffix;?>">
					</div>
					...
					<label <?php  if($item['inkind'] == 0) { ?>style="display:none;"<?php  } else { ?>style="display:inline-block;"<?php  } ?> class="num">数量：<input type="text" class="form-control" value="<?php  echo $item['total'];?>" style="width:80px;float:right;" name="award-total<?php  echo $namesuffix;?>"></label> 
				</div>
			</td>
		</tr>
		<tr>
			<th>奖品描述：</th>
			<td>
				<textarea style="height:80px;" name="award-description<?php  echo $namesuffix;?>" class="form-control" cols="70" placeholder="填写奖品描述，颜色、类型、规格等" id=""><?php  echo $item['description'];?></textarea>
			</td>
		</tr>
		...
		<tr>
			<th>兑 换 码：</th>
			<td>
				<textarea style="height:80px;" class="form-control" cols="70" placeholder="请一行填写一个兑换码或者其他密文类SN码（每个中奖码只能被使用一次）" id="" name="award-activation-code<?php  echo $namesuffix;?>"><?php  echo $item['activation_code'];?></textarea>
			</td>
		</tr>
		<tr>
			<th>激活地址：</th>
			<td>
				<input type="text" id="" class="form-control" placeholder="填写激活地址或者其他领奖方法" value="<?php  echo $item['activation_url'];?>" name="award-activation-url<?php  echo $namesuffix;?>">
			</td>
		</tr>
		<?php  } ?>
	</table>
