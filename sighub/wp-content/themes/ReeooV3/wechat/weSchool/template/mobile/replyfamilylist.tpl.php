...
<html>
	...
	<form id="reply" onSubmit="return contentcheck();" action="" method="post">		
			...
			<?php 
				...
				//分页逻辑处理
				//-----------------------------------------------------------------------------------------------
				foreach($countreplys as $replyfamily){
					 $countreplyfamily=$replyfamily['Creply'];
				}
				//echo "这是老师的个数：".$countteacher;
				$pages=intval($countreplyfamily/$pagesize); //计算总页数

				if ($countreplyfamily % $pagesize) $pages++;

				//设置缺省页码
				//↓判断“当前页码”是否赋值过
				if (isset($_GET['replyfamily'])){ $page=intval($_GET['replyfamily']); }else{ $page=1; }//否则，设置为第一页
			   
				//↓计算记录偏移量
					$offset=$pagesize*($page - 1);

				//↓读取指定记录数
				$familyreply=$this -> doWebReplyfamily($offset,$pagesize,$gweid,$notice_id);//取得—当前页—记录集！
					
			?>
			
			<?php foreach($familyreply as $freply){ ?>
					<div style="margin-top: 5px; border-bottom: 1px solid #E2E2E2;font-size: 18px;">
					<?php echo $freply['reply_content']; ?>
					<div>
					<span style="font-size: 13px;"><i><font color=DodgerBlue>  
					<?php if(substr($freply['reply_author'],0,1)=='t'){
								$teachername=$this -> doWebReplyfamilytname(substr($freply['reply_author'],1));
								foreach($teachername as $tname){
									echo $tname['tea_name']." 回复于：".$freply['reply_time'];
								}
							}else{
								$familyname=$this -> doWebReplyfamilysname(substr($freply['reply_author'],1));
								foreach($familyname as $fname){
								echo $fname['stu_name']." 回复于：".$freply['reply_time'];
							}
						} ?>
					</font></i></span>
					</div>
			...
			<?php
				...
				if ($page > 1)
				{
					echo "<a href='{$this->createMobileUrl('replyfamilylist',array('replyfamily' => $first, 'no' => $no,'gweid' => $gweid))}'>首页</a>  &nbsp;&nbsp;&nbsp";
					echo "<a href='{$this->createMobileUrl('replyfamilylist',array('replyfamily' => $prev, 'no' => $no,'gweid' => $gweid))}'>上一页</a>  ";
					
				}

				if ($page < $pages)
				{
					echo "<a href='{$this->createMobileUrl('replyfamilylist',array('replyfamily' => $next, 'no' => $no,'gweid' => $gweid))}'>下一页</a>  &nbsp;&nbsp;&nbsp";
					echo "<a href='{$this->createMobileUrl('replyfamilylist',array('replyfamily' => $last, 'no' => $no,'gweid' => $gweid))}'>尾页</a>  ";
