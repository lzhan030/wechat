...
<div id="primary" class="site-content">
	<form action=" "  method="post">
		<div style="margin-left:10px;">
			...
			<div class="panel panel-default" style="margin-left:30px;margin-right:30px; margin-top:20px">
				...
				<?php 
                    ...
                    $postsCount=web_admin_count_post($sid,"page");//获取记录总数						
					...
					//↓读取指定记录数
					//echo "offset=".$offset;
					$rs=web_admin_array_post($offset,$pagesize,$sid,"page");//取得—当前页—记录集！
						
					//一个function活的总个数
					//foreach (){$curNem=iii->as bianliang}
					//$curNum = mysql_num_rows($rs); //$curNum - 当前页实际记录数，for循环输出用
							
					$arraysCount=web_admin_array_post_count($offset,$pagesize,$sid,"page");//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
				...
				{/*如果没有http，证明为内链，加上home_url显示，插入时，判断有再截取入库*/
					...
					if(($tmp===false)&&(!empty($mov2_item->guid))){
						$menulink=home_url().$mov2_item->guid;
					}else{				
						$menulink=$mov2_item->guid;
					}
					echo "<tr>";
					echo "	<td><input type='checkbox' id='myCheck' name='inputCheckBox' value='".$menulink."'/></td>";
					echo "	<td>$mov2_item->post_title</td>";
					echo "	<td>$mov2_item->post_date</td>";
					echo "<td><input type='button' onClick='deletePost({$mov2_item->ID})' name='del' id='buttondel' value='删除' class='btn btn-sm btn-info' style='margin-right: 10px;'/>"  ;
					echo "<input type='button' onClick='updatePost({$mov2_item->ID})' name='upd' id='buttonupd' value='修改' class='btn btn-sm btn-warning' /> </td>" ;
					echo "</tr>	";
				}
				?>
		...
		<div align=right style="margin-right:30px"> 
			<input type='button'  id='buttonqd' value='确定' class="btn btn-primary" style="width:120px" onclick='OK();'/> 
			<input type='button'  id='buttoncle' value='取消' class="btn btn-default" style="width:120px" onclick='Cancle();'/> 
		</div>
