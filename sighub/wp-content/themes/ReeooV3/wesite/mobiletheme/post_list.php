...
<?php
	...
	//判断是否是mobiletheme1或者 mobiletheme5，然后判断相应的相应的gweid是否开启sync up的功能
	//20141019查看该gweid是否需要syncup功能,
	$gweidsql = $wpdb -> prepare("SELECT * FROM {$wpdb->prefix}orangesite where id=%d",$siteId);
	$getgweids = $wpdb->get_results($gweidsql);
	foreach($getgweids as $getgweid)
	{
	    $gweid = $getgweid -> GWEID;
		$themes = $getgweid -> themes_key;	  //只有模板8需要同步功能
	} 
	if($themes == 8)   //如果是模板8，重置addAttachment的值为空，否则添加文章的时候执行的函数有问题
	{
	    $site_pic = "";
	}
		...
		<form action="" method="post">
			<div>
				...
				<div>
					<td>
						<a href="#"
							onclick="javascript:window.open('<?php echo get_template_directory_uri(); ?>/wesite/common/post_insert_dialog.php?beIframe&artType=post&addAttachment=<?php echo $site_pic ?>&refreshOpener=yes&siteId=<?php echo $siteId ?>','_blank','height=562,width=888,top=120,left=240,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no,titlebar=no')">
					...
					<div class="panel panel-default" style="margin-right:50px; margin-top:30px">
						...
						//		$numAL = mysql_num_rows($site_list);  //取得记录总数$rs
							...
							<?php foreach($rs as $mobile_item){
								...
								if($themes == 8){
									$sqlstatus = $wpdb -> prepare("SELECT * FROM wp_postmeta WHERE post_id = %d and meta_key = 'syncup_status'", $mobile_item->ID);
									$getstatus = $wpdb->get_results($sqlstatus);
									
									if(empty($getstatus))   //之前添加失败，此时取出的值为null
									{
									    echo "	<td>同步失败</td>";
									}else{
							...
							?>		
							<a href="#" onclick="javascript:window.open('../common/post_insert_dialog.php?beIframe&artType=<?php echo "page" ?>&refreshOpener=yes&siteId=<?php echo $siteId?>&postid=<?php echo $mobile_item->ID ?>','_blank','height=562,width=888,top=120,left=240,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no,titlebar=no')">
						    <input type="button" class="btn btn-sm btn-info" onClick=  id="menu" value="修改"/>
			                </a>
							<a href="#" onclick="javascript:window.open('<?php echo get_template_directory_uri(); ?>/wesite/mobiletheme/comment_manage.php?beIframe&postid=<?php echo $mobile_item->ID ?>','_blank','height=520,width=800,top=120,left=240,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no,titlebar=no')">
...
<script language='javascript'>
	...
	function deletePost(ID){	   
		if(confirm("确定删除吗？")){
			...
			xmlHttp.open("GET","<?php echo constant("CONF_THEME_DIR"); ?>/wesite/mobiletheme/post_delete.php?beIframe&header=0&footer=0&postid="+ID,true);
			xmlHttp.onreadystatechange = function(){
				if(xmlHttp.readyState == 4 && xmlHttp.status == 200)
				{    
				   //alert("删除成功");
				   alert(xmlHttp.responseText);	
				   location.href="<?php echo constant("CONF_THEME_DIR"); ?>/wesite/mobiletheme/post_list.php?beIframe&siteId="+"<?php echo $siteId;?>";
				}	
