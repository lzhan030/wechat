...
<?php
	...
	/*2014-06-27删除 
	$wids = 0;
	$getwids=getswid($_SESSION['WEID'],$user_id);
	foreach($getwids as $getwid){
		$wids = $getwid->wid;
	}
	$wechatactivity_vip=web_admin_function_info($_SESSION['WEID'],"wechatvip",$wids,$user_id);
	$wechatschool=web_admin_function_info($_SESSION['WEID'],"wechatschool",$wids,$user_id);  *///判断是否选择微学校服务
			...
			<?php foreach($menu as $menu_info){
				foreach($menu_url as $menu_url_info){
					foreach($menu_img as $menu_img_info){
						...
						//$member=$_SERVER['HTTP_HOST']."/?vip_detail";
