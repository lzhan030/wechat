...
<?php
	...
	//该模板是mobilethemesyncup，有sync up的功能
		...
		<form action="" method="post">
			<div>
				...
				<div>
					<td>
						<a href="#"
							onclick="javascript:window.open('<?php echo get_template_directory_uri(); ?>/wesite/common/post_insert_dialog.php?beIframe&artType=post&addAttachment=<?php echo $site_pic ?>&refreshOpener=yes&siteId=<?php echo $siteId ?>','_blank','height=520,width=800,top=120,left=240,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no,titlebar=no')">
					...
					<div class="panel panel-default" style="margin-right:50px; margin-top:30px">
						...
						//		$numAL = mysql_num_rows($site_list);  //取得记录总数$rs
							...
							<?php foreach($rs as $mobile_item){
								...
								echo " <td><input type='button' onClick='deletePost({$mobile_item->ID},{$siteId})' name='del' class='btn btn-sm btn-warning' id='buttondel' value='删除' class='btn_add'> " ;
							...
							?>		
							<a href="#" onclick="javascript:window.open('../common/post_insert_dialog.php?beIframe&artType=<?php echo "page" ?>&refreshOpener=yes&siteId=<?php echo $siteId?>&postid=<?php echo $mobile_item->ID ?>','_blank','height=520,width=800,top=120,left=240,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no,titlebar=no')">
		...
		if(confirm("确定删除吗？")){
			...
			xmlHttp.onreadystatechange = function(){
				...
				{    
				   ...
				   location.href="<?php echo constant("CONF_THEME_DIR"); ?>/wesite/mobiletheme/post_list.php?beIframe&siteId="+siteid;
				}	
