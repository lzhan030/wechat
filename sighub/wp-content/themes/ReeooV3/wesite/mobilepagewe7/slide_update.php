...
<?php
	...
	if ($_FILES["file"]["error"] > 0){
		if($delimgid!=-1){
			web_admin_update_menu($slideid,$menu_title, null, $imgid,$inserturl,$urlid);
			$wpdb -> update($wpdb -> prefix.'site_nav',array(
				'name' => $menu_title,
				'url' => $inserturl
			),array(
			'id' => $slideid
			...
			web_admin_update_menu($slideid,$menu_title, -1, $imgid,$inserturl,$urlid);
			$wpdb -> update($wpdb -> prefix.'site_nav',array(
				'name' => $menu_title,
				'url' => $inserturl,
				'icon' => null
