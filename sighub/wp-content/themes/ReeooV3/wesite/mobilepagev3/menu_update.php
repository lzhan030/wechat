...
<?php
	...
	if($_POST["menuUrl"]==0){				
		...
		//修改时默认为外联，改为内联
		if($menuiUrl==null){
			$menuiUrl=$_SESSION['post_url'];
			unset($_SESSION['post_url']);
		}
	...
	if ($_FILES["file"]["error"] > 0){
		web_admin_update_menu3($menuid,$menu_title, null, $imgid,$inserturl,$urlid);
	}else{
		...
		if($picUrl!=false){
			...
			web_admin_update_menu3($menuid,$menu_title, $insert_imgid, $imgid,$inserturl,$urlid);
