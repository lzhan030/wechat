...
elseif(!current_user_can('level_10')){
	...
	wp_redirect(home_url().'/login?redirect_to='.$_SERVER["REQUEST_URI"]."&error=NotAdmin");
}
*/
...
{  /* include(home_url().'/?admin&page=usermanage');  */
?>
...
<?php
	...
	if(isset($_GET['del']) && !empty($_GET['del']) )
	{
		...
		$username = $wpdb->get_var($wpdb->prepare("SELECT user_login from ".$wpdb->prefix."users WHERE ID = %d",$_GET['del']));
		if(!empty($username))
		{
