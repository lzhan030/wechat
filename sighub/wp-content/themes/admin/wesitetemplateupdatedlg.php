<?php
	...
	global $wpdb;
	$id = $_GET['id'];
	$wesite = web_admin_getnewtemplate($id);
?>
...
<div>
	<div class="main-title">
		<div class="title-1">当前位置：模板管理 > <font class="fontpurple">修改微官网模板 </font>
		</div>
	</div>
	<form name="myForm" id="wesitetmpadd" action="?admin&page=cgi-bin/wesite_template_update" method="post" onsubmit="return validationform();" enctype="multipart/form-data">
	<div style="width: 80%; margin-top:50px; margin-left:30px;">	
		<div class="row clearfix">
			<div class="col-xs-3 col-md-3 column"><label class="floatright">模板编号：</label></div>
			<div class="col-xs-9 col-md-9 column"><?php echo $wesite->id;?><input type="hidden" name="id" value=<?php echo $wesite->id;?> /></div>
		</div>
		<div class="row clearfix">
			<div class="col-xs-3 col-md-3 column"><label class="floatright"><font color="red">* </font>模板名称：</label></div>
			<div class="col-xs-9 col-md-9 column"><input class="form-control" type="text" name="title" value="<?php echo $wesite->title;?>" /></div>
		</div>	
		<div class="row clearfix">
			<div class="col-xs-3 col-md-3 column"><label class="floatright">是否启用：</label></div>
			<div class="col-xs-2 col-md-2 column">
				<input type="radio" name="activate" value="1" <?php if($wesite->activate == 1) {?> checked="checked" <?php }?> >启用
			</div>
			<div class="col-xs-7 col-md-7 column">
				<input type="radio" name="activate" value="0" <?php if($wesite->activate == 0) {?> checked="checked" <?php }?> >禁止
			</div>
		</div>	
		<div class="row clearfix">
			<div class="col-xs-3 col-md-3 column"><label class="floatright">是否有背景图片：</label></div>
			<div class="col-xs-2 col-md-2 column">
				<input type="radio" name="background" value="1" <?php if($wesite->background == 1) {?> checked="checked" <?php }?> > 有
			</div>
			<div class="col-xs-7 col-md-7 column">
				<input type="radio" name="background" value="0" <?php if($wesite->background == 0) {?> checked="checked" <?php }?> > 无
			</div>
		</div>
		<div class="row clearfix">
			<div class="col-xs-3 col-md-3 column"><label class="floatright">是否支持幻灯片：</label></div>
			<div class="col-xs-2 col-md-2 column">
				<input type="radio" name="slide" value="1" checked="checked" <?php if($wesite->slide == 1) {?> checked="checked" <?php }?> > 支持
			</div>
			<div class="col-xs-7 col-md-7 column">
				<input type="radio" name="slide" value="0" <?php if($wesite->slide == 0) {?> checked="checked" <?php }?> > 不支持
			</div>
		</div>	
		<div class="row clearfix">
			<div class="col-xs-3 col-md-3 column"><label class="floatright">是否支持菜单：</label></div>
			<div class="col-xs-2 col-md-2 column">
				<input type="radio" name="menu" value="1" <?php if($wesite->menu == 1) {?> checked="checked" <?php }?> > 支持
			</div>
			<div class="col-xs-7 col-md-7 column">
				<input type="radio" name="menu" value="0" <?php if($wesite->menu == 0) {?> checked="checked" <?php }?> > 不支持
			</div>
		</div>	
		<div class="row clearfix">
			<div class="col-xs-3 col-md-3 column"><label class="floatright">是否支持菜单背景颜色：</label></div>
			<div class="col-xs-2 col-md-2 column">
				<input type="radio" name="menu_bg" value="1" <?php if($wesite->menu_bg == 1) {?> checked="checked" <?php }?> > 支持
			</div>
			<div class="col-xs-7 col-md-7 column">
				<input type="radio" name="menu_bg" value="0" <?php if($wesite->menu_bg == 0) {?> checked="checked" <?php }?> > 不支持
			</div>
		</div>			
		<div class="row clearfix">
			<div class="col-xs-3 col-md-3 column"><label class="floatright">是否支持菜单图片：</label></div>
			<div class="col-xs-2 col-md-2 column">
				<input type="radio" name="image_icon" value="1" <?php if($wesite->image_icon == 1) {?> checked="checked" <?php }?> > 支持
			</div>
			<div class="col-xs-7 col-md-7 column">
				<input type="radio" name="image_icon" value="0" <?php if($wesite->image_icon == 0) {?> checked="checked" <?php }?>> 不支持
			</div>
		</div>	
		<div class="row clearfix">
			<div class="col-xs-3 col-md-3 column"><label class="floatright">模板代码的zip文件：</label></div>
