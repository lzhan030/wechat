...
$endurl=$suburl[0].'&site='.$siteId.'&gweid='.$gweid.'#'.$suburl[1];

$location = empty($_POST['redirect_to']) ? $endurl: $_POST['redirect_to'] . '#comment-' . $comment_id;
//$location = empty($_POST['redirect_to']) ? get_comment_link($comment_id).'&site='.$siteId.'&mid='.$mid.'&fromuser='.$fromuser: $_POST['redirect_to'] . '#comment-' . $comment_id;
//end
