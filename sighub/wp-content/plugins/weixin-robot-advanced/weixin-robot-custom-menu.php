...
function weixin_robot_post_custom_menus($weixin_robot_custom_menus){
	...
	if($weixin_robot_basic['weixin_app_id'] && $weixin_robot_basic['weixin_app_secret']){
		...
		if($weixin_robot_access_token){
			...
			$result = weixin_robot_post_custom_menus_core($request,$url);
		}
	}

	do_action('weixin_robot_post_custom_menus',$weixin_robot_custom_menus);	

	return $result;
}

function weixin_robot_post_custom_menus_core($request,$url){
...
function weixin_robot_custom_menu_list(){
	...
	<h2>自定义菜单<a href="<?php echo admin_url('admin.php?page='.$plugin_page.'&sync'); ?>" class="add-new-h2">同步自定义菜单</a></h2>
