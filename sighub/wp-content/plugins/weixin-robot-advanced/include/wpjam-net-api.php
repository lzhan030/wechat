...
function wpjam_net_admin_menu() {
	...
	if($wpjam_net_count === false){
		...
		set_transient('wpjam_net_count',array('update_plugins_count'=>$update_plugins_count, 'update_domains_count'=>$update_domains_count),300);
	}
	...
	$update_info = '';
	if($total_count = $update_domains_count + $update_plugins_count > 0){
...
function wpjam_net_page(){
	...
	<div class="wrap">
		...
		<table class="widefat" cellspacing="0">
			...
			<tbody>
			<?php $thickbox_content = ''; ?>
			<?php foreach ($wpjam_net_plugin_datas->items as $item) {?>
				<tr>
					<td><img src="<?php echo $item->thumb; ?>" width="50" /></td>
					<td><a href="<?php echo admin_url('admin.php?page='.$plugin_page.'&inlineId=item_'.$item->id.'&TB_inline&width=500&height=600'); ?>" class="thickbox"><?php echo $item->name; ?></a></td>
					<td><?php echo $item->sections['description']; ?></td>
...
function wpjam_net_check_plugin_status($local_file,$remote_item){
	...
	if(!wpjam_net_check_domain($remote_item->id)){
		$status = '你的域名还没有经过授权';
		$action = '<a href="http://wpjam.net/wp-admin/admin.php?page=orders&domain_limit=1&product_id=$id" class="button button-primary" target="_blank">授权域名</a>';
	}elseif($new_version > $current_version){
		...
		$action = '<a href="http://wpjam.net/wp-admin/admin.php?page=orders" class="button button-primary" target="_blank">已授权</a>';
	}
